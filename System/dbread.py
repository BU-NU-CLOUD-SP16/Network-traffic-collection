...
# FOR THE CASE WHERE REQUESTS ARE STREAMED FROM tcpdump
if __name__ == "__main__":
	...
	jobs = [] # to store results of all the db write processes
	array = [] # for temporarily storing requests in memory
	M = 0	# for system state per run of program
	index = 0
	count = 0
	pcap_header_fmt = ['gmt_time', 'micro_time', 'pcap_len', 'len']
	ip_header_fmt = ['version,ihl', 'tos', 'tot_len', 'id', 'frag_off', 'ttl', 'protocol', 'check', 'saddr', 'daddr']
	...
	while True:
		...
		# time now in micro seconds for accuracy
			...
			if pcap_len > 14+ip_head_length+tcp_head_length:
				...
				checker = data.find('\r\n')
				if(checker != -1):
					httpchecker = data.find('HTTP', 0, checker)
					if httpchecker != -1:
						extract_http(data, dict_field)
						count += 1
						# return the dict_field so it can be written
						array.append(dict_field)
			# check if max load size reach, if so, write to DB
			newM = system.streamcheck(count,M)
			if(newM > M and M !=0):
				p = Process(target=monitor.writestream, args=(count,array,M))
				jobs.append(p)
				p.start()
				M = newM
				count = 0
				array = []
	monitor.writestream(count,array,M)